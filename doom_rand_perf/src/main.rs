...
const RAND_CALL_NUMBER: u32 = 10_000_000;
...
fn main() {
    let start_doom_rand = Instant::now();
    for _ in 0..RAND_CALL_NUMBER {
        let a = doom_rand();
        let b = a.max(128);
        assert!(b > 127);
    }
    let duration_doom_rand = start_doom_rand.elapsed();

    let start_os_rand = Instant::now();
    for _ in 0..RAND_CALL_NUMBER {
        let a = os_rand();
        let b = a.max(128);
        assert!(b > 127);
    }
    let duration_os_rand = start_os_rand.elapsed();

    let start_thread_rand = Instant::now();
    for _ in 0..RAND_CALL_NUMBER {
        let a = thread_rand();
        let b = a.max(128);
        assert!(b > 127);
    }
    let duration_thread_rand = start_thread_rand.elapsed();

    let formatted_rand_call_number = RAND_CALL_NUMBER.to_formatted_string(&Locale::en);

    println!(
        "Time for {} calls of Doom rand : {:?}",
        formatted_rand_call_number, duration_doom_rand
    );
    println!(
        "Time for {} calls of OS rand : {:?}",
        formatted_rand_call_number, duration_os_rand
    );
    println!(
        "Time for {} calls of Thread rand : {:?}",
        formatted_rand_call_number, duration_thread_rand
    );
}
